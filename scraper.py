...
def clean_text(text):
    # Remove extra spaces and unwanted elements
    text = re.sub(
        r"NRK TVNRK RadioNRK SuperNRK P3Yr.*?MerNyheter", "", text
    )  # Example removal
...
def extract_article(text):
    # Extract the article text from the provided text
...
if __name__ == "__main__":
    ...
    # Optionally, extract all article links (hrefs)
...
"""     searchResults = soup.select("body > #kurator_main")
    if searchResults:
        kurator_main = searchResults[0]
        section_count = len(kurator_main.find_all("section"))
        print(f"Number of <section> elements under #kurator_main: {section_count}")

        for i in kurator_main.find_all("section"):
            if i.get_text(strip=True) == "":
                continue
            print(i.get_text(strip=True))
            print(len(i.get_text(strip=True)))
    else:
        print("No element with id 'kurator_main' found.") """
